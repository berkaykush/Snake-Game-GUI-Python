            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    for button in self._buttons:
                        if button.get_top_rect.collidepoint(pygame.mouse.get_pos()):
                            button.change_color(
                                top_rect_color=COLORS['RED'], bottom_rect_color=COLORS['DARK RED'])
                            continue

                        button.change_color(
                            top_rect_color=COLORS['BLUE'], bottom_rect_color=COLORS['DARK BLUE'])

                elif event.type == pygame.MOUSEBUTTONDOWN:
                    for button in self._buttons:
                        if button.get_top_rect.collidepoint(pygame.mouse.get_pos()):
                            button.set_elevation(0)

                elif event.type == pygame.MOUSEBUTTONUP:
                    for button in self._buttons:
                        if button.get_elevation == 0:
                            button.set_elevation(10)
                            ...
                            if button.get_top_rect.collidepoint(pygame.mouse.get_pos()):
                                if button == self._buttons[0]:
                                    ...
                                    self._reset_score()
                                    running = False
                                elif len(self._buttons) > 1 and button == self._buttons[1]:
                                    self._quit_game()
